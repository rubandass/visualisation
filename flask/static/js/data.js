$(document).ready(function () {
    ...
    // Get values for the selected country by AJAX call method
    function getValue(selected_country, slider_year, filterByCountry) {
        $.get("/countries/".concat(selected_country),
            function (response) {
                if (response.status == "success") {
                    let responseObj = response.country_data;
                    let industry_gdp = responseObj['industry_percent_of_gdp'] || [];
                    let agriculture_gdp = responseObj['agriculture_percent_of_gdp'] || [];
                    let services_gdp = responseObj['services_percent_of_gdp'] || [];
                    ...
                    if (filterByCountry) {
                        for (key in industry_gdp) {
                            // To reduce the x axis bandwidth, datas will be provided for every 3 years
                            if (key % 3 == 0) {
                                gdp_models.push({
                                    "year": key,
                                    "field1": parseFloat(industry_gdp[key] || 0),
                                    "field2": parseFloat(agriculture_gdp[key] || 0),
                                    "field3": parseFloat(services_gdp[key] || 0)
                                });
                            }
                        };
                    } else {
